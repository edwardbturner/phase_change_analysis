...
def main():
    ...
    print(f"Train data size: {len(train_data[0])}, batches per epoch: {len(train_loader)}")
    print("Logging cos(grad, H@grad) every 200 steps\n")
    
    step = 0
    epoch = 0
    
    # Use tqdm for step counter, not epoch
    pbar = tqdm(total=50000, desc='Training steps')
    ...
    while step < 50000:
        ...
        for inputs, labels in train_loader:
            ...
            if step % 200 == 0:
                ...
                with torch.no_grad():
                    # Use current batch for analysis
                    batch_inputs, batch_labels = inputs, labels
                ...
                if cos_sim is not None:
                    ...
                    with torch.no_grad():
                        ...
                        # Test accuracy (sample for speed)
    ...
    pbar.close()
    print(f"\nCompleted {step} steps over {epoch} epochs")
