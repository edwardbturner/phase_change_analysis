...
def plot_training_metrics(
    metrics: Dict[str, List[float]], save_path: Optional[str] = None, title: str = "Training Metrics"
) -> None:
...
def plot_eigenvalue_evolution(
    eigenvalues: np.ndarray, steps: List[int], save_path: Optional[str] = None, top_k: int = 10
) -> None:
...
def plot_phase_transition(
    metrics: Dict[str, np.ndarray], transition_point: Optional[int] = None, save_path: Optional[str] = None
) -> None:
    ...
    for train_key, test_key, title, ax in metric_pairs:
        ...
        if transition_point:
            ax.axvline(transition_point, color="red", linestyle="--", label="Phase Transition")
