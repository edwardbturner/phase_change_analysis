...
def train_induction_heads(args):
    ...
    test_loader = create_induction_data_loader(
            ...
            for batch_idx, (batch_sequences, batch_targets) in enumerate(train_loader):
                ...
                if step % args.log_interval == 0:
                    ...
                    train_metrics = compute_metrics(
                        model, train_loader, train_pattern_info, device
                    )
                    test_metrics = compute_metrics(
                        model, test_loader, test_pattern_info, device
                    )
                    
                    # Track eigenspectrum
                    if step % args.eigen_interval == 0 and step > 0:
                        print(f"\n[Step {step}] Computing eigenspectrum...")
                        eigen_analyzer.track_training_step(
                            hessian_batch, step, test_metrics['pattern_accuracy']
                        )
    ...
    for layer_idx in range(args.n_layers):
        for head_idx in range(min(4, args.n_heads)):  # Analyze first 4 heads
            analysis = visualize_attention_patterns(
                model, test_sequences, test_pattern_info,
                layer_idx=layer_idx, head_idx=head_idx, seq_idx=0
            )
            if analysis:
