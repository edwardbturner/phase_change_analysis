...
class HessianEigenAnalyzer:
    ...
    def _compute_hvp(
        ...
        # Compute grad @ vector
        grad_dot_vec = sum((g * v).sum() for g, v in zip(grads, vec_params))
    ...
    def compute_gradient_hessian_alignment(
        self,
        data_batch: Tuple[torch.Tensor, torch.Tensor]
    ) -> Dict:
        """
    ...
    def analyze_attention_heads(self, model: nn.Module) -> Dict:
        ...
        for layer_idx in range(model.n_layers):
            ...
            ov_off_diag_strength = torch.mean(
                torch.abs(OV - torch.diag(torch.diag(OV)))
            ).item()
            
            attention_analysis[f'layer_{layer_idx}'] = {
                'qk_top_eigenvalue': qk_eigenvalues[-1].item(),
                'qk_eigenvalue_gap': (qk_eigenvalues[-1] - qk_eigenvalues[-2]).item(),
                'ov_top_eigenvalue': ov_eigenvalues[-1].item(),
                'ov_eigenvalue_gap': (ov_eigenvalues[-1] - ov_eigenvalues[-2]).item(),
    ...
    def track_training_step(
        ...
        self.eigenvalue_history.append({
            ...
            'top_eigenvalues': alignment_info['eigenvalues'][:5].tolist() if len(alignment_info['eigenvalues']) >= 5 else alignment_info['eigenvalues'].tolist(),
